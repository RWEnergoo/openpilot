...
def get_mads_limited_brands(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params: Params) -> bool:
  if CP.brand == 'rivian':
    return True
  if CP.brand == 'tesla':
    # Full MADS needs a consistent way to switch lateral off. Either escape hatch qualifies:
    # the fork's cruise-button cancel, or upstream's configurable screen button.
    if CP_SP.flags & TeslaFlagsSP.BUTTON_CANCELS:
      return False
